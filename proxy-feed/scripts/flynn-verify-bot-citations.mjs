...
// Usage: FLYNN_TOKEN=... BOOKING_FIZIKA_LOGIN=... BOOKING_FIZIKA_PASSWORD=... [SINCE=2026-05-24T10:33] [LIMIT=30] node ...
...
function parsePortalPage(html) {
  ...
  const priceMatch = text.match(/Стоимость\s*(\d[\d  ]{4,})\s*₽/i) || text.match(/(\d[\d  ]{4,})\s*₽\s*\d+\s*\d+\s*₽\/м/);
...
function parseBotClaim(line) {
  ...
  // Price MUST be preceded by comma+space (separates it from "эт. 4, ").
  const priceM = line.match(/,\s*((?:\d{1,3}(?:[\s  ]\d{3})+|\d{4,}))\s*₽/);
  const floorM = line.match(/(?:^|[\s,])эт\.?\s*(\d+)/i);
...
const normComplex = s => (s||'').toLowerCase()
  .replace(/жк\s+/g,'')
  .replace(/\s+(апартаменты|апарт|резиденции?|первых)/g,'')
  .replace(/\s+\d+/g,'')
  .replace(/[^a-zа-я]/gi,'');
...
async function main() {
  ...
  const msgsUrl = `https://flynn-ai.ru/rest/v1/messages?select=content,visitor_id,created_at&project_id=eq.${PROJECT_ID}&role=eq.assistant${sinceFilter}&order=created_at.desc&limit=${LIMIT}`;
  ...
  for (const p of pairs) {
    ...
    const issues = [];
    if (p.claim.area != null && portal.area && Math.abs(p.claim.area - portal.area) > 0.5) issues.push(`area: ${p.claim.area} vs ${portal.area}`);
    if (p.claim.price != null && portal.price && Math.abs(p.claim.price - portal.price) > 50000) issues.push(`price: ${p.claim.price} vs ${portal.price}`);
    if (p.claim.floor != null && portal.floor && p.claim.floor !== portal.floor) issues.push(`floor: ${p.claim.floor} vs ${portal.floor}`);
    if (p.claim.rooms != null && portal.rooms != null && p.claim.rooms !== portal.rooms) issues.push(`rooms: ${p.claim.rooms} vs ${portal.rooms}`);
    if (p.claim.complex && portal.complex && normComplex(p.claim.complex) !== normComplex(portal.complex)) issues.push(`complex: ${p.claim.complex} vs ${portal.complex}`);
    if (issues.length === 0) ok++; else mismatches.push({ ...p, portal, issues });
  }
  console.log(`\nVerified: ${pairs.length}, MATCH: ${ok}, MISMATCH: ${mismatches.length}\n`);
  for (const m of mismatches) {
    console.log(`[${m.when}] ${m.url}`);
    console.log(`  Bot:    ${m.line}`);
