...
const COMPLEX = {
  talento:    { id: 183, full: 'Talento',         min: 'talento' },
...
function detectRooms(q) {
  ...
  if (/однушк|однокомн|1-комн|одна\s+комн/.test(l)) return 1;
  if (/двушек|двушк|двух|2-комн/.test(l)) return 2;
  if (/трёшк|трешк|трёх|трех|3-комн/.test(l)) return 3;
  if (/четыр|4-комн/.test(l)) return 4;
  return null;
}
...
function sliceUrl(complex, rooms, budget) {
  const params = [];
  if (rooms !== null && rooms !== undefined) params.push('rooms=' + (rooms === 0 ? '%D1%81%D1%82' : rooms));
  if (budget) params.push('maxPrice=' + budget);
  const q = params.length ? '?' + params.join('&') : '';
  if (!complex) {
    return Object.values(COMPLEX).map(c => `https://booking.fizika.group/catalog/${c.id}/${q}`).join('  |  ');
  }
  return `https://booking.fizika.group/catalog/${COMPLEX[complex].id}/${q}`;
}
function __sliceUrlOld() {
