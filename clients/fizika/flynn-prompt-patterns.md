...
## ❌ АНТИПАТТЕРН (зафиксировано 2026-05-24): KB-обходка через синонимы

**Этот раздел оставлен как историческое предупреждение.** Подход «лечить fallback добавлением KB-карточки со списком синонимов» — антипаттерн. Список формулировок бесконечен, любая новая фраза → провал.

**Правильный подход:** правила структуры запроса (один критерий / count / atribut-filter) — в **системный промпт** (логика). KB — только **данные** (адреса, диапазоны цен, фактическое состояние объектов).

Подробнее: `docs/rag-bot-setup-principles.md` раздел «Антипаттерн 1».

Финальное состояние Fizika: правила вынесены в промпт через PostgREST (6886 знаков, обход UI-лимита 4000), KB-карточки с синонимами удалены. См. `baseline-2026-05-24.md`.

---

## 9. Историческое описание KB-обходки (НЕ ПОВТОРЯТЬ)
